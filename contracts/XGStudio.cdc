...
pub contract XGStudio: NonFungibleToken {
    ...
    pub resource AdminResource: NFTMethodsCapability {
        ...
        pub fun createTemplate(brandId: UInt64, maxSupply: UInt64, immutableData: {String: AnyStruct}, mutableData:{String: AnyStruct}?) {
            ...
            emit TemplateCreated(templateId: XGStudio.lastIssuedTemplateId, brandId: brandId, maxSupply: maxSupply)
        ...
        pub fun updateTemplateMutableData(templateId: UInt64, mutableData: {String: AnyStruct}) {
            pre{
                XGStudio.whiteListedAccounts.contains(self.owner!.address): "you are not authorized for this action"
                XGStudio.allTemplates[templateId] != nil: "Template Id does not exists"        
            }

            let oldTemplate = XGStudio.allTemplates[templateId]
            if self.owner?.address! != XGStudio.allBrands[oldTemplate!.brandId]!.author {
                panic("No permission to update others Template's Mutable Data")
            }

            XGStudio.allTemplates[templateId]!.updateMutableData(mutableData: mutableData)
        ...
        pub fun updateTemplateMutableAttribute(templateId: UInt64, key: String, value: AnyStruct) {
            ...
            XGStudio.allTemplates[templateId]!.updateMutableAttribute(key: key, value: value)
            emit TemplateUpdated(templateId: templateId)
        }
