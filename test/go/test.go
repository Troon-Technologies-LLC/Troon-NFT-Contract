...
func CadenceUInt64(value uint64) cadence.Value {
	newValue := cadence.NewUInt64(value)

	return newValue
}

func CadenceArray(value uint64) cadence.Array {
	newValue := cadence.NewArray([]cadence.Value{cadence.NewUInt64(value)})
	return newValue
}

func CadenceInt(value int) cadence.Int {
	newValue := cadence.NewInt(value)
	return newValue
}

func CadenceString(name string) cadence.String {
	newValue, _ := cadence.NewString(name)
	return newValue
}

func CadenceDictionary(value []cadence.KeyValuePair) cadence.Dictionary {
	newValue := cadence.NewDictionary(value)
